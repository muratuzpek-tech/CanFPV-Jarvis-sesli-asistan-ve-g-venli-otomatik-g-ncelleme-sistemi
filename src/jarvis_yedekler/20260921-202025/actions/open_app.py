...
try:
    import psutil  # noqa: F401  (yalnizca varlik kontrolu)
    _PSUTIL = True
except ImportError:
    _PSUTIL = False
...
def _launch_windows(app_name: str) -> bool:
    # Never pass model/user input through a command shell.  In particular,
    # names containing &, |, ; or redirection syntax must remain one argv item.
    ...
    if ":" in app_name:
        try:
            # Windows URI/file associations are opened without cmd.exe.
            os_startfile = getattr(__import__("os"), "startfile", None)
            if os_startfile is None:
                return False
            os_startfile(app_name)
            time.sleep(1.0)
            return True
        except Exception:
            pass
