...
def _execute_generated_code(code: str, player=None) -> str:
    # SECURITY: LLM-generated Python is untrusted input.  A restricted
    # ``exec`` namespace is not a real sandbox: Python object introspection can
    # escape it, and injected pyautogui/ctypes objects provide direct desktop
    # control.  Keep this legacy entry point, but never execute model output.
    return (
        "LLM-generated desktop code execution is disabled for safety. "
        "Use an explicit, allowlisted desktop action instead."
    )
