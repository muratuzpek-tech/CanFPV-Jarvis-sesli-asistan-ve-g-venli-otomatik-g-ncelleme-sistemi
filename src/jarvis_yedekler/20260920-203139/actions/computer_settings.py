...
def _xrandr_brightness(delta: float) -> bool:
    """Adjust the first connected XRandR output without invoking a shell."""
    try:
        query = subprocess.run(["xrandr", "--query"], capture_output=True, text=True, timeout=5)
        output = next((line.split()[0] for line in query.stdout.splitlines()
                       if " connected" in line), None)
        if not output:
            return False
        ...
        for line in verbose.stdout.splitlines():
            if "Brightness:" in line:
                try:
                    current = float(line.split("Brightness:", 1)[1].split()[0])
                    break
                except (ValueError, IndexError):
                    pass
        value = max(0.1, min(1.0, current + delta))
        result = subprocess.run(["xrandr", "--output", output, "--brightness", f"{value:.3f}"],
                                capture_output=True, text=True, timeout=5)
