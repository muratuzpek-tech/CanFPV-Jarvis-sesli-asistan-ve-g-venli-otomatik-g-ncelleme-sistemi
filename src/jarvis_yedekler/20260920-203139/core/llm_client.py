...
def _load_config() -> dict:
    try:
        ...
        data = {}
    # Environment variables override the local file, which avoids committing keys.
...
def call_llm(
    ...
    try:
        ...
        raise RuntimeError("Ollama request timed out after 120 s.") from _exc
...
def _stream_openai(
    ...
    try:
        ...
        raise RuntimeError("OpenAI-compatible stream timed out.") from _exc
    except requests.exceptions.HTTPError as e:
...
def call_llm_stream(
    ...
    try:
        ...
        raise RuntimeError("Ollama stream timed out.") from _exc
    except requests.exceptions.HTTPError as e:
        raise RuntimeError(f"Ollama HTTP error: {e.response.status_code}") from e
    except Exception as e:
