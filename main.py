...
TOOL_DECLARATIONS = [
    ...
    },
    {
        "name": "self_heal",
        "description": (
            "JARVIS self-healing system: automatically diagnose and fix errors in JARVIS own codebase. "
            "Use when JARVIS encounters an internal error, crash, or malfunction. "
            "Can fix dependency errors, syntax errors, import errors, runtime errors, "
            "connection errors, and API errors. Also can check self-heal history. "
            "Actions: fix (auto-repair), diagnose (analyze error), status (check history), health (check all modules health)."
        ),
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "action": {
                    "type": "STRING",
                    "description": "fix | diagnose | status | health (default: fix)"
                },
                "error": {
                    "type": "STRING",
                    "description": "The error message or traceback to fix"
                }
            },
            "required": ["action"]
        }
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        try:
            if name == "open_app":
                ...
                r = await loop.run_in_executor(None, lambda: self_heal(parameters=args, player=self.ui, speak=self.speak))
                result = r or "Done."
